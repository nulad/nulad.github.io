...
describe('Homepage', () => {
  ...
  describe('when posts are available', () => {
    ...
    it('should display post metadata correctly', async () => {
      ...
      // Check that "Read more" text exists (using getAllByText since there are multiple)
      expect(screen.getAllByText('Read more')).toHaveLength(3);

      // Check link href for first post (using getAllByTestId and getting the first one)
      const postLinks = screen.getAllByTestId('post-link');
      expect(postLinks[0].closest('a')).toHaveAttribute('href', '/posts/test-post-1');
    });
