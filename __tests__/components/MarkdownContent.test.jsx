...
const mockSanitize = vi.fn((html) => {
  ...
  if (!stripped || stripped.trim().length === 0) {
    // Check if it has meaningful elements
    if (!/<(img|hr|br|input|table)/i.test(html)) {
      return "";
    }
  }

  if (
    html.includes('<input type="checkbox" checked disabled> Completed task')
  ) {
    return html;
  }
...
describe("MarkdownContent", () => {
  ...
  describe("Component styling", () => {
    ...
    test("applies custom className when provided", () => {
      const markdown = "# Title";
      const { container } = render(
        <MarkdownContent content={markdown} className="custom-class" />
      );
